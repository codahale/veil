...
\begin{abstract}
    ...
    services for messages of arbitrary sizes and multiple receivers. Veil is implemented as a
    command line tool \texttt{veil}\@. This document describes its feature set and usage.
\end{abstract}
...
required, and \texttt{veil} does not provide those assurances. It's more of an art installation than
a practical tool.
...
\texttt{./my-private-key}\@. That's it. There's no user IDs, no key signing, no key servers, no
banging on the keyboard to generate entropy.
...
decrypt if they have your public key. It adds 18 fake receivers, so neither receiver really knows
how many people you sent the message to. It also adds 1234 bytes of random padding, so someone
monitoring your communications won't know how long the message really is.
