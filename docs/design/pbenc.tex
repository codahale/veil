...
\begin{algorithm}[ht]
    ...
    \begin{algorithmic}
        \Function{EncryptPrivateKey}{$P, M_{cost}, T_{cost}, d$}
            ...
            \State $K \gets $\Call{Argon2id}{\texttt{0x13}, $P_N$, $S$, $M_{cost}$, $T_{cost}$}\Comment{Hash the passphrase.}
...
\begin{algorithm}[ht]
    ...
    \begin{algorithmic}
        \Function{DecryptPrivateKey}{$P, C=M_{cost} || T_{cost} || S || C || T$}
            \State $P_N \gets$\Call{NormalizeNFC}{$P$}\Comment{Normalize the passphrase.}
            \State $S \rgets \allbits{128}$\Comment{Generate a random salt.}
            \State $K \gets $\Call{Argon2id}{\texttt{0x13}, $P_N$, $S$, $M_{cost}$, $T_{cost}$}\Comment{Hash the passphrase.}
            \State \Call{Absorb}{\texttt{veil.pbenc}}\Comment{Initialize an unkeyed duplex.}
...
\texttt{veil.pbenc} integrates Argon2, a memory-hard key derivation function, and a standard Cyclist authenticated
encryption scheme.
