...
\begin{algorithm}[ht]
    \caption{
        Encrypting a private key $d$ given a passphrase $P$, memory cost parameter $C_M$, time cost parameter $C_T$, and
        parallelism cost parameter $C_P$.
    }
    \begin{algorithmic}
...
\texttt{veil.pbenc} integrates Argon2~\cite{rfc9106}, a memory-hard key derivation function, and a standard Cyclist
authenticated encryption scheme.
Argon2 increases the cost for an attacker to mount a brute-force or dictionary attack using highly parallel hardware
\@(e.g.\ GPUs, ASICs) by increasing the per-core memory requirements for attempts.
