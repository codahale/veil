...
\begin{algorithm}[ht]
    ...
    \begin{algorithmic}
        ...
        \Procedure{InitFromPassphrase}{$P, S, N_T, N_S$}
            ...
            \State $P_N \gets$\Call{NormalizeNFKC}{$P$}\Comment{Normalize the passphrase.}
            ...
            \For{$t$ in $0..N_T$}
                ...
                \For{$m$ in $0..N_S$}\Comment{Hash last and current blocks.}
                    ...
                    \For{$i$ in $0..D$}\Comment{Hash in pseudo-randomly chosen blocks.}
                        \State $I \gets $\Call{Hash}{$S, t || m || i$}$ \bmod N_S$
                        \State $B[m] \gets $\Call{Hash}{$B[m], B[I]$}
                    \EndFor
                \EndFor
            \EndFor
            \State
            \State \Call{Cyclist}{$B[N_S-1], \epsilon, \epsilon$}\Comment{Extract output from buffer.}
        \EndProcedure
    \end{algorithmic}
