...
\begin{algorithm}[!htp]
    ...
    \begin{algorithmic}
        \Function{Sign}{$(d_S, Q_S), M$}
            \State \Call{Cyclist}{\texttt{veil.schnorr}, $\epsilon$, $\epsilon$}\Comment{Initialize duplex with constant key.}
            \State \Call{Absorb}{$Q$}\Comment{Absorb the signer's public key.}
            \State
            \ForAll{$M_i \in M$}
                \State \Call{Absorb}{$M_i$, $16$}\Comment{Absorb message in 32KiB blocks.}
            \EndFor
            \State
