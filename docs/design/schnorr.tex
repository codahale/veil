...
challenge scalar $r$ and calculating $r'$.
In this way, \texttt{veil.schnorr} is closer to EdDSA~\cite{brendel2021} or the Schnorr variant
proposed by Hamburg~\cite{hamburg2017}.
...
\begin{displayquote}
    ...
    applying the Fiat-Shamir transform to it.
...
permutation is indistinguishable from a random permutation.
Thus, \texttt{veil.schnorr} is UF-CMA if the discrete-logarithm problem is hard relative to jq255e
and Keccak-\emph{p} is indistinguishable from a random permutation.
...
multiple valid signatures for a given signer and message~\cite{brendel2021}.
\textcite{chalkias2020} describe a strict verification function for Ed25519 which achieves sUF-CMA
security in addition to strong binding:
