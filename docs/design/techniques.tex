...
Veil uses Xoodyak, which offers both unkeyed (``hash'') and keyed modes.
All Veil constructions begin in the unkeyed mode by absorbing a constant domain separation string (e.g.\
\texttt{veil.mres}).
To convert from an unkeyed duplex to a keyed duplex, a 256-bit key is derived from the unkeyed duplex's state and used
to initialize a keyed duplex:

\begin{algorithm}[ht]
    \caption{Converting an unkeyed Xoodyak duplex to a keyed duplex.}
    \begin{algorithmic}
        ...
        \State $K \gets $ \Call{SqueezeKey}{$32$}\Comment{Squeeze a 256-bit key.}
...
The unkeyed duplex is used as a kind of key derivation function, with the lower absorb rate of Xoodyak's unkeyed mode
