...
\begin{algorithm}[ht]
    ...
    \begin{algorithmic}
        \Function{EncryptMessage}{$(d_S, Q_S), Q_{R^0}..Q_{R^n}, N_P, P$}
            ...
            \End
            \State
            \State $C \gets $\Call{PointToUniform}{$Q_E$}\Comment{Encode the key with Elligator Squared.}
            ...
            \State
            \State $r \gets$ \Call{Squeeze}{$32$} $\modl$\label{alg:veil.mres-encrypt-challenge}\Comment{Squeeze a challenge scalar.}
...
\begin{algorithm}[ht]
    ...
    \begin{algorithmic}
        \Function{DecryptMessage}{$(d_R, Q_R), Q_S, C$}
            ...
            \State
            \State \Call{Absorb}{$C[0..64]$}\Comment{Absorb the encoded ephemeral public key.}
            \State $Q_E \gets $\Call{UniformToPoint}{$C[0..64]$}\Comment{Decode the ephemeral public key.}
            \State
            \ForAll{possible encrypted headers $h \in C$}
                \State $N \gets$\Call{Squeeze}{$16$}
                \State $x \gets $ \Call{DecryptHeader}{$(d_R, Q_R), Q_S, Q_E, h, N$}
                \If{$x \not= \bot$}
...
The ephemeral public key is encoded using Elligator Squared, which maps points on an elliptic curve to uniform
bitstrings~\cite{tibouchi2014}.
All other components of an \texttt{veil.mres} ciphertext are Cyclist ciphertexts;
...
the use of Cyclist allows for a significant reduction in complexity.
