...
\begin{algorithm}[ht]
    \caption{
        ...
        length $N_{pad}$, and plaintext $P$.
    }
    \begin{algorithmic}
        \Function{EncryptMessage}{$(d_S, Q_S), Q_{R^0}..Q_{R^n}, N_{pad}, P$}
            ...
            \End
            \State
            \State $Q_E = [d_E]G$\Comment{Calculate and mask the ephemeral public key.}
            \State $C \rgets $\Call{Mask}{$Q_E$}
            \State
            \State $H = K||(N_{hdr}+N_{pad})$\Comment{Encode the DEK and message offset in a header.}
            \ForAll{$Q_{R^i} \in \{Q_{R^0}..Q_{R^n}\}$}
                \Comment{Encrypt the header for each receiver.}
                \State $C \gets C||$\Call{EncryptHeader}{$(d_S, Q_S), (d_E, Q_E), Q_{R^i}, H$}
            \EndFor
            \State
            \State $y \rgets \allbits{N_{pad}}$\Comment{Append random padding.}
            \State $C \gets C||y$
            \State \Call{AbsorbMore}{$C, 16$}\Comment{Absorb key, headers, and padding.}
            \State
            \State \Call{Cyclist}{$K$, $\epsilon$, $\epsilon$}\Comment{Re-key the duplex with the DEK.}
            \ForAll{32-KiB blocks $p \in P$}
                \State $C \gets C||$\Call{Encrypt}{$p$}
                \State $C \gets C||$\Call{Squeeze}{$16$}\Comment{Authenticate each block with a tag.}
                \State \Call{Ratchet}{}
            \EndFor
            \State
            \State $I \gets [k]G$\Comment{Calculate the commitment point.}
            \State $C \gets C||$\Call{Encrypt}{$I$}\Comment{Encrypt the commitment point.}
            \State
            ...
            \State $C \gets C||$\Call{Encrypt}{$s$}\Comment{Encrypt the proof scalar.}
...
\begin{algorithm}[ht]
    ...
    \begin{algorithmic}
        \Function{DecryptMessage}{$(d_R, Q_R), Q_S, C$}
            ...
            \State $Q_E \gets $\Call{Unmask}{$C_{0..31}$}\Comment{Unmask and decode the ephemeral public key.}
            \State
            \ForAll{possible encrypted headers $h \in C$}
                \State $x \gets $ \Call{DecryptHeader}{$(d_R, Q_R), Q_S, Q_E, h$}
                \If{$x \not= \bot$}
                    \Comment{Break if a header is successfully decrypted.}
                    \State $K || (N_{hdr}+N_{pad}) \gets x$
                    \State \textbf{break}
                \EndIf
            \EndFor
            \State
            \State \Call{AbsorbMore}{$C_{0..{32+N_{hdr}+N_{pad}}}, 16$}\Comment{Absorb key, headers, and padding.}
            \State $C \gets C_{{32+N_{hdr}+N_{pad}}..}$\Comment{Skip to the message beginning.}
            \State
            \State \Call{Cyclist}{$K$, $\epsilon$, $\epsilon$}\Comment{Re-key the duplex with the DEK.}
            \State
