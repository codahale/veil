...
\begin{algorithm}[ht]
    ...
    \begin{algorithmic}
        \Function{EncryptMessage}{$(d_S, Q_S), Q_{R^0}..Q_{R^n}, N_P, P$}
            ...
            \State $C \gets $\Call{Encrypt}{$Q_E$}\Comment{Encrypt the ephemeral public key.}
            \State
...
\begin{algorithm}[ht]
    ...
    \begin{algorithmic}
        \Function{DecryptMessage}{$(d_R, Q_R), Q_S, C$}
            ...
            \State $Q_E \gets $\Call{Decrypt}{$C[0..32]$}\Comment{Decrypt the ephemeral public key.}
            \State
...
\texttt{veil.mres} ciphertexts are indistinguishable from random noise provided the adversary does not know the sender's
public key.
If the adversary knows the sender's public key, they can use it to decrypt the ephemeral public key and distinguish
valid messages with high probability.
Otherwise, all components of an \texttt{veil.mres} ciphertext are Xoodyak ciphertexts;
...
Swapping the \textsc{Decrypt} operation to produce an ephemeral key pair with a suitable uniform representative would
not affect its security properties and would eliminate the available distinguishing attack.
