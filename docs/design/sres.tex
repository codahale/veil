...
\begin{algorithm}[!htp]
    ...
    \begin{algorithmic}[1]
        \Function{DecryptHeader}{$(d_R, Q_R), Q_S, Q_E, N, C||S_0||S_1$}
            ...
            \State
            \If{$\pm I \checkeq \pm ([d_R^{-1}]X \pm [r]Q_S) $}\Comment{Ensure the ciphertext is authentic.}
...
and a designated-verifier qDSA signature scheme.
...
The latter portion of \texttt{veil.sres} is a designated-verifier Schnorr signature scheme which adapts a qDSA-style
...
The qDSA-style Schnorr signature is sUF-CMA secure when $s$ is restricted to positive values (see
