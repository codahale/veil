...
headers.
It integrates an ephemeral Diffie-Hellman KEM, a Xoodyak AEAD, and a designated-verifier Schnorr signature
...
\begin{algorithm}[!htp]
    \caption{
        ...
        $Q_R$, plaintext $P$, and authenticated data $D$.
    }
    \begin{algorithmic}[1]
        \Function{EncryptHeader}{$(d_S, Q_S), (d_E, Q_E), Q_R, P, D$}
            \State \Call{Cyclist}{\texttt{veil.sres}, $\epsilon$, $\epsilon$}\Comment{Initialize duplex with constant key.}
            ...
            \State \Call{Absorb}{$D$}\Comment{Absorb the authenticated data.}
            \State

            \State \Call{Cyclist}{$d_S[Q_E]$, $\epsilon$, $\epsilon$}\Comment{Re-key duplex with ephemeral shared secret.}
            ...
            \State
            \State $r \gets$ \Call{SqueezeKey}{$64$} $\modl$
...
\begin{algorithm}[!htp]
    \caption{
        ...
        $Q_E$, ciphertext $C||S_0||S_1$, and authenticated data $D$.
    }
    \begin{algorithmic}[1]
        \Function{DecryptHeader}{$(d_R, Q_R), Q_S, Q_E, C||S_0||S_1, D$}
            \State \Call{Cyclist}{\texttt{veil.sres}, $\epsilon$, $\epsilon$} \Comment{Initialize duplex with constant key.}
            ...
            \State \Call{Absorb}{$D$}\Comment{Absorb the authenticated data.}
            \State
            \State \Call{Cyclist}{$d_R[Q_E]$, $\epsilon$, $\epsilon$}\Comment{Re-key duplex with ephemeral shared secret.}
...
(with $D$ as authenticated data, the commitment point $I$ as an addition to the ciphertext, and the challenge scalar $r$
serving as the authentication tag for the data encapsulation mechanism) and is IND-CCA2 secure (see Corollary 12.14
