...
headers.
It integrates an ephemeral Diffie-Hellman KEM, a Xoodyak DEM, and a designated-verifier Schnorr signature
...
\begin{algorithm}[!htp]
    ...
    \begin{algorithmic}[1]
        \Function{EncryptHeader}{$(d_S, Q_S), (d_E, Q_E), Q_R, N, P$}
            ...
            \State \Call{Cyclist}{$d_S[Q_E]$, $\epsilon$, $\epsilon$}\Comment{Re-key duplex with the shared secret.}
...
\begin{algorithm}[!htp]
    ...
    \begin{algorithmic}[1]
        \Function{DecryptHeader}{$(d_R, Q_R), Q_S, Q_E, N, C||S_0||S_1$}
            ...
            \State \Call{Absorb}{$N$}\Comment{Absorb the nonce.}
            \State
            \State \Call{Cyclist}{$d_R[Q_E]$, $\epsilon$, $\epsilon$}\Comment{Re-key duplex with the shared secret.}
