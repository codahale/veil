...
complex protocols. It combines SHA-256 and Rocca-S to provide ~10 GiB/sec performance on modern
...
that either SHA-256 is not collision-resistant or that Rocca-S is not PRF secure.

### Indistinguishability From Random Noise

Given that both signature components are encrypted with Rocca-S, an attack which distinguishes
between a `veil.schnorr` and random noise would also imply that Rocca-S is distinguishable from a
random function.
...
`S₁`--would only be possible if either SHA-256 is not collision-resistant or Rocca-S is not PRF
secure.
...
All of the components of a `veil.sres` ciphertext--`C₀`, `C₁`, `S₀`, and `S₁`--are Rocca-S
...
distinguishing attack would imply that either SHA-256 is not collision-resistant or that Rocca-S is
not PRF secure.
...
either SHA-256 is not collision-resistant or Rocca-S is not PRF secure.
...
ciphertext are Rocca-S ciphertexts; a successful distinguishing attack on them would imply that
SHA-256 is not collision-resistant or Rocca-S is not PRF secure.
